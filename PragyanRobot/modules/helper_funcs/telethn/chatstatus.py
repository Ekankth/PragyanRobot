...
from PragyanRobot import DRAGONS
from PragyanRobot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def pragyan_is_admin(chat_id: int):
    status = False
    pragyan = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if pragyan.id == user.id:
            status = True
            break
    return status
