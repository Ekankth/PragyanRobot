...
import PragyanRobot.modules.sql.global_bans_sql as sql
...
from PragyanRobot.modules.helper_funcs.misc import send_to_list
from PragyanRobot.modules.sql.users_sql import get_user_com_chats
...
def gban(update: Update, context: CallbackContext):
    ...
    try:
        bot.send_message(
            ...
            f"</b>Appeal Chat:</b> @{PRAGYANIITIANCHAT}",
            parse_mode=ParseMode.HTML,
        )
    except:
...
def __user_info__(user_id):
    ...
    if is_gbanned:
        ...
        if user.reason:
            text += f"\n<b>Reason:</b> <code>{html.escape(user.reason)}</code>"
        text += f"\n<b>Appeal Chat:</b> @{PRAGYANIITIANCHAT}"
    else:
        text = text.format("???")
    return text
