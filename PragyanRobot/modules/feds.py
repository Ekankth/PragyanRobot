...
import PragyanRobot.modules.sql.feds_sql as sql
...
from PragyanRobot.modules.helper_funcs.alternate import send_message
from PragyanRobot.modules.helper_funcs.chat_status import is_user_admin
from PragyanRobot.modules.helper_funcs.extraction import (
    extract_unt_fedban,
    extract_user,
    extract_user_fban,
)
from PragyanRobot.modules.helper_funcs.string_handling import markdown_parser
...
def new_fed(update: Update, context: CallbackContext):
    ...
    if not fednam == "":
        ...
        if not x:
            update.effective_message.reply_text(
                f"Can't federate! Please contact @{PRAGYANIITIANCHAT} if the problem persist.",
            )
            return
...
def join_fed(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if not x:
            message.reply_text(
                f"Failed to join federation! Please contact @{PRAGYANIITIANCHAT} should this problem persist!",
            )
            return
...
def fed_ban(update: Update, context: CallbackContext):
    ...
    if fban:
        ...
        if not x:
            message.reply_text(
                f"Failed to ban from the federation! If this problem continues, contact @{PRAGYANIITIANCHAT}.",
            )
            return
...
def set_frules(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if not x:
            update.effective_message.reply_text(
                f"Whoa! There was an error while setting federation rules! If you wondered why please ask it in @{PRAGYANIITIANCHAT}!",
            )
            return
